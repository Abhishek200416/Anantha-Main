...
# ============= RAZORPAY SETTINGS (API KEYS MANAGEMENT) =============
...
async def get_razorpay_settings(current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        logger.error(f"Error fetching Razorpay settings: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Failed to fetch Razorpay settings: {str(e)}")

@api_router.put("/admin/razorpay-settings")
async def update_razorpay_settings(
    ...
    try:
        ...
        # Reinitialize Razorpay client with new credentials
        ...
        logger.error(f"Error updating Razorpay settings: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Failed to update Razorpay settings: {str(e)}")
